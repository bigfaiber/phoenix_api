class UsersController < ApplicationController
  ...
  def index
    ...
    investors = Investor.where('lower(name) LIKE ? OR lower(lastname) LIKE ? OR lower(identification) LIKE ?', pattern, pattern, pattern).pluck(:id, :name, :lastname, :identification).map{ |i| { id: i[0], fullname: "#{i[1]} #{i[2]}", identification: i[3], type: 'Investor', status: '' } }
